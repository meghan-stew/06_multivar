...
weather %>%
  ...
  ungroup() %>%
  slice(1:6) %>% 
  # slice pulls first 6 rows which are the top six for r2
  unnest(data) %>%
  ggplot(aes(x = value, 
             y = strength_gtex))+
  geom_point(shape = 21, 
  ...
  facet_wrap(~name, 
             scales = "free_x", 
             ncol=2) 
# will plot strength by weather variables
...
lm_dayl.apr <- lm(strength_gtex ~ mean_dayl.s_Apr, data = weather) %>%
  tidy() %>%
...
lm_dayl.apr %>%
  bind_rows(lm_prp.oct, 
            lm_dayl.prcp,
            lm_daylapr.daylmay) %>%
  # binding rows of second model to the third model to combine them into one object for plotting, but need a way to name which model each data point comes from hence "mod" column
  ...
  # model column should be in specific order on x axis for ggplot
  # easy way to change order of vectors in dataset going into ggplot is using mutate, where levels are used to write the sequence you want them to show up for plotting. Default is alphabetical not high to low. Medium here should be in middle. Plot level changes should be changed at data set level using mutate! 
  ggplot(aes(x=mod)) +
